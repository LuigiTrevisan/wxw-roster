...
export const RosterFilters = ({ filters, setFilters }) => {
  return (
    <div className="flex flex-wrap justify-center gap-4">
      ...
      <button
        onClick={() =>
          setFilters((prev) => ({ ...prev, gender: prev.gender === 'male' ? null : 'male' }))
        }
        className={`flex items-center px-4 py-2 sm:px-6 sm:py-3 rounded-md border-2 text-sm sm:text-lg font-medium transition-all duration-300 ${
          filters.gender === 'male'
      ...
      <button
        onClick={() =>
          setFilters((prev) => ({ ...prev, gender: prev.gender === 'female' ? null : 'female' }))
        }
        className={`flex items-center px-4 py-2 sm:px-6 sm:py-3 rounded-md border-2 text-sm sm:text-lg font-medium transition-all duration-300 ${
      ...
      <button
        onClick={() =>
          setFilters((prev) => ({
            ...prev,
            showTagTeams: prev.showTagTeams === true ? false : true,
          }))
        }
        className={`flex items-center px-4 py-2 sm:px-6 sm:py-3 rounded-md border-2 text-sm sm:text-lg font-medium transition-all duration-300 ${
          filters.showTagTeams === true
            ? 'bg-red-500/20 border-red-500 text-white'
            : 'bg-gray-900/90 border-gray-700 text-gray-400 hover:border-red-500/50 hover:text-white'
        }`}
      >
